...
void solve() {
    ...
    int max = 0;
    // str max_search = "";
    int size_diff = size_s - size_t;
    rep(s_start_i, size_diff+1) {
        //書き換える文字数
        //書き換える場所
        // sにあてる位置をずらす
        int count = 0;
        // str target_s = s.substr(s_start_i, size_t);
        // str target_t = t.substr(0, size_t);
        rep(si, size_t) {
            // char check_s = s[s_start_i+si];
            // char check_t = t[si];
            if (s[s_start_i+si] == t[si]) {
                count++;
            }
        }
        chmax(max, count);
