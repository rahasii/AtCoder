...
str deleteSameString(str S, int start) {
    int size = S.size();
    // if (size > 1 && S[size - 1] == S[size - 2]) {
    //     S.erase(size - 1);
    //     S.erase(size - 2);
    //     deleteSameString(S);
    // }
    _repi(i, start, S.size()) {
        if (S[i + 1] > 0 && S[i] == S[i + 1]) {
            S.erase(i);
            S.erase(i + 1);
            deleteSameString(S, i);
        }
    }
