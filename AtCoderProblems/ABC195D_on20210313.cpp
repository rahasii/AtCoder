...
template <typename T>
void dec(vector<T> &v) { rep(i, v.size()) v[i]--; }
template <typename T>
...
void solve()
{
    in(int, N_bag, N_box, N_query);

    vector<map<str, int>> bags(N_bag);
    rep(i, N_bag)
    {
        in(int, width, value);
        bags[i]["width"] = width;
    ...
    vector<map<str, int>> boxies(N_bag);
    rep(i, N_box)
    {
        in(int, width);
        bags[i]["width"] = width;
    ...
    {
        int tmp_value = 0;
        vector<map<str, int>> condition = boxies;
        in(int, L, R);
        condition.erase(L, R);

        rep(j, bags.size())
        {
            rep(k, condition.size())
            {
                if(bags[j]["width"]<= condition[k]["width"]){
                    // int tmp_value += 
    ...
    //最大カンマの数
    double max = (to_string(N).size() - 1) / 3;
    ll count = 0;
    rep(i, max)
    {
        ll k = pow(10, 3 * (i + 1));
